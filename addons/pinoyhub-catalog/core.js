/**
 * PinoyHub Catalog — Stremio-protocol catalog addon engine
 * Source site: pinoymovieshub.win (WordPress + Dooplay 2.5.5)
 * Author: xrexzerox
 * Version: 1.0.0
 *
 * WHAT THIS IS
 ...
 *   such an addon: it scrapes pinoymovieshub.win listing pages, resolves every
 *   title to a TMDB id (so Nuvio can open detail pages and route getStreams
 *   calls to the PinoyMoviesHub plugin), and serves:
 *
 *     GET /manifest.json
...
(function (global) {
  ...
  // Cached listing-page fetch + parse + type filter. Returns item[].
  function fetchSitePageCached(cfg, url, type) {
    ...
    return fetchText(cfg, url, 20000).then(function (html) {
  ...
  function indexHtml(cfg) {
    ...
    return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>PinoyHub Catalog</title>' +
      ...
      '<h1>PinoyHub Catalog <small>v' + VERSION + '</small></h1>' +
      '<p>Stremio-protocol catalog addon for <a href="' + cfg.site + '">pinoymovieshub.win</a>, built for Nuvio.</p>' +
      ...
      '<h2>Extras</h2><p>Search: <code>/catalog/movie/pinoy-movies/search=cobra kai.json</code> &middot; Genre: <code>/catalog/series/pinoy-series-genre/genre=Tagalog Dubbed&amp;skip=20.json</code></p>' +
      '<p>Pair with the <b>PinoyMoviesHub</b> Nuvio plugin (providers/pinoyhub.js in xrexzerox/nv-plugins) for playable streams.</p>' +
      '</body></html>';
  }
